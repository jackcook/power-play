analyze_points <- function(data) {
  ...
  for (i in 1:length(coordinates_strs[[1]])) {
    parts <- unlist(strsplit(coordinates_strs[[1]][i], ","))
  ...
  for (i in 1:(length(coordinates) / 2 - 1)) {
    shots <- c(shots, analyze_shot(coordinates[i,], coordinates[i + 1,]))
  }
}
